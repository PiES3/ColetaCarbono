...
# Schema específico para aprovação/rejeição pelo Gestor
class AtualizarStatusEmpresa(BaseModel):
    novo_status: StatusVinculo
    motivo_recusa: str | None = None

@router.post("/", response_model=EmpresaResponse, status_code=status.HTTP_201_CREATED)
def autocadastro_empresa(empresa_in: EmpresaCreate, db: Session = Depends(get_db)):
    """
    ...
    # AVISO: Numa implementação real, a palavra-passe deve passar por uma função de hash (ex: Passlib)
...
def validar_vinculo_empresa(
    ...
    """
    Endpoint para o Gestor de Resíduos aprovar ou recusar o vínculo de uma empresa (HU003).
    ...
    if dados_status.novo_status == StatusVinculo.RECUSADA and not dados_status.motivo_recusa:
